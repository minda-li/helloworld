...
__device__ float sigma_function(float* sigmas, int row, int col, int num_cols) {
    // Fetch the corresponding sigma value from the sigmas matrix
    return sigmas[row * num_cols + col];
}

__global__ void divide_kernel(float *W, float *Q, float *sigmas, int num_rows, int num_cols, int c, float beta, float gamma, int g, int k) {
    ...
    if (row < num_rows && col < c) {
        ...
        // Denominator using sigma function
        float sigma_value = sigma_function(sigmas, row, col, num_cols);
        float denominator = W[row * num_cols + col] + beta * sigma_value + gamma;

        // Calculate Q
        Q[row * c + col] = numerator / denominator;

        // Debugging output
        printf("row: %d, col: %d, W: %f, g^i: %f, k^j: %f, numerator: %f, sigma: %f, denominator: %f, Q[%d, %d]: %f\n",
               row, col, W[row * num_cols + col], g_power, k_power, numerator, sigma_value, denominator, row, col, Q[row * c + col]);
    }
}
...
__global__ void prefix_product_kernel(float *QQ, float *P, int total_size) {
    ...
    if (idx < total_size) {
        if (idx == 0) {
            ...
            shared_mem[threadIdx.x] = QQ[idx - 1];
            __syncthreads();
...
int main() {
    ...
    // Sigmas matrix initialization (populate this according to the assignment)
    float sigmas[num_rows * m] = { /*...fill in with actual sigma values...*/ };
    ...
    float *d_W, *d_Q, *d_QQ, *d_P, *d_sigmas;
    ...
    cudaMalloc((void **)&d_sigmas, num_rows * m * sizeof(float));

    // Copy data to device
    cudaMemcpy(d_W, W, num_rows * m * sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(d_sigmas, sigmas, num_rows * m * sizeof(float), cudaMemcpyHostToDevice);
    ...
    divide_kernel<<<gridSize, blockSize>>>(d_W, d_Q, d_sigmas, num_rows, m, c, beta, gamma, g, k);
